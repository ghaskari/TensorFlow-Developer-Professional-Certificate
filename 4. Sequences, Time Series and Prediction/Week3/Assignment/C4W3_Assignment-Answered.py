...
def create_uncompiled_model():
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=-1), input_shape=[None]),  # Expand dimensions for RNN input
        tf.keras.layers.SimpleRNN(40, return_sequences=True),  # First RNN layer
        tf.keras.layers.SimpleRNN(40),  # Second RNN layer
...
def adjust_learning_rate():
    ...
    # Assuming `dataset` is already defined and properly prepared
