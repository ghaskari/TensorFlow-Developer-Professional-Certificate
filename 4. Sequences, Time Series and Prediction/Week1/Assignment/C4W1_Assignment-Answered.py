...
time = np.arange(4 * 365 + 1)  # 4 years of daily data
baseline = 10
...
# Adjust time_valid to match the length of series_valid and naive_forecast
adjusted_time_valid = time_valid[:len(series_valid)]
...
# Plot validation series and naive forecast for specific range
plt.figure(figsize=(10, 6))
plot_series(adjusted_time_valid, series_valid, label="validation set")
plot_series(adjusted_time_valid, naive_forecast, label="naive forecast")
plt.legend()
plt.show()
...
time_valid = time_valid[:len(series_valid)]
...
diff_moving_avg = diff_moving_avg[-len(series_valid):]
...
# Adjusted to align correctly with the differenced moving average
past_series = SERIES[len(SERIES) - len(diff_moving_avg):]
...
# Apply exponential moving average to the past series
smooth_past_series = exponential_moving_average(pd.Series(SERIES[split_time - 370: split_time + 1]), 10)

print(f"smooth past series has shape: {smooth_past_series.shape}\n")

# Ensure smooth_past_series and diff_moving_avg are aligned
smooth_past_series = smooth_past_series[-len(diff_moving_avg):]

# Add the smoothed past values to the moving avg of diff series
